...
COLORS = ["yellow", "red", "green", "orange",
          "gray", "pink", "purple", "tan", "olive"]

SELECTED_CLASS = 0
selected_class_indexes = [i for i in range(len(CLASSES))]
selected_class_indexes_as_strs = [
    str(index + 1) for index in selected_class_indexes]  # used for easy indexing
...
class ImageCanvas(tk.Canvas):
    def __init__(self, *args, **kwargs):
        ...
        self.resizeable_image = self.resizeable_image.resize(
            (1200, 800), Image.ANTIALIAS)
        self.resized_photoimage = ImageTk.PhotoImage(self.resizeable_image)
        self.image_on_canvas = self.create_image(
            0, 0, anchor=tk.NW, image=self.resized_photoimage, tag="all")
    ...
    def save_labels(self, image_filename, labels):
        ...
        with open(label_filename, "w") as f:
            ...
            f.write("\n".join([" ".join(yolo_label)
                               for yolo_label in yolo_formatted_labels]))
            ...
            if os.path.exists(label_filename):
                ...
                with open(label_filename, "r") as yolo_label_file:
                    ...
                    for yolo_label in yolo_labels:
                        ...
                        x, y, width, height = float(x), float(
                            y), float(width), float(height)
                        class_name = CLASSES[class_index]
                        bb = [x, y, x+width, y+height]
                        bb_id = self.create_rectangle(bb[0] * self.winfo_width(), bb[1] * self.winfo_height(
                        ), bb[2] * self.winfo_width(), bb[3] * self.winfo_height(), fill="", outline=COLORS[class_index])
                        new_label_text_id = self.create_text(
                            x * self.winfo_width(), (y * self.winfo_height()) - 5, fill=COLORS[class_index], text=class_name)
                        self.labels.append(
                            [bb, class_name, bb_id, new_label_text_id])
        print("labels:")
        for label in self.labels:
            print(label)
    ...
    def load_image(self, filename):
        ...
        self.resizeable_image = Image.open(self.image_filename).resize(
    ...
    def update_crosshair(self, event):
        ...
        cross_hair_color = "red" if self.editing else CROSS_HAIR_COLORS[
            SELECTED_CROSS_HAIR_COLOR_INDEX]

        self.horizontal_dash_line = self.create_line(
            0, event.y, self.winfo_width(), event.y, fill=cross_hair_color, dash=(5, 2))
        self.vertical_dash_line = self.create_line(
            event.x, 0, event.x, self.winfo_height(), fill=cross_hair_color, dash=(5, 2))
    ...
    def move_mouse(self, event):
        ...
        if self.new_label_clicked_xy != None:
            ...
            x2 = min(max(event.x, 0), self.winfo_width())
            y2 = min(max(event.y, 0), self.winfo_width())
            self.new_label_temporary_box = self.create_rectangle(
                x1, y1, x2, y2, fill="", outline=COLORS[SELECTED_CLASS], dash=(5, 2))
            self.new_label_temporary_text = self.create_text(
                x1, y1 - 5, fill=COLORS[SELECTED_CLASS], text=CLASSES[SELECTED_CLASS])
    ...
    def click_release(self, event):
        ...
        if self.new_label_clicked_xy != None and self.image_filename != DEFAULT_IMAGE_FILENAME:  # Create Box
            ...
            self.new_label_box = self.create_rectangle(
                x1, y1, x2, y2, fill="", outline=COLORS[SELECTED_CLASS])
            self.new_label_text = self.create_text(
                x1, y1 - 5, fill=COLORS[SELECTED_CLASS], text=CLASSES[SELECTED_CLASS])
            bounding_box = (min(x1, x2) / self.winfo_width(), min(y1, y2)/self.winfo_height(),
                            max(x1, x2)/self.winfo_width(), max(y1, y2)/self.winfo_height())
            new_label = (
                bounding_box, CLASSES[SELECTED_CLASS], self.new_label_box, self.new_label_text)
...
class App(tk.Tk):
    ...
    def create_widget_frame(self):
        ...
        tk.Label(widget_frame, text='Adding boxes: ').grid(
            label_config, row=current_row)
    ...
    def hotkey(self, event):
        if event.char in selected_class_indexes_as_strs:
            ...
            if len(self.frames) > 0:
                ...
                self.frame_canvas.load_image(
                    self.frames[self.current_frame_index])
                self.filename_label.config(
                    text="Current File: " + self.frames[self.current_frame_index])
        elif event.char == "a":
            if len(self.frames) > 0:
                self.current_frame_index = (
                    self.current_frame_index - 1) % len(self.frames)
            ...
            cross_hair_color = "red" if self.frame_canvas.editing else CROSS_HAIR_COLORS[
                SELECTED_CROSS_HAIR_COLOR_INDEX]
            self.frame_canvas.itemconfig(
                self.frame_canvas.horizontal_dash_line, fill=cross_hair_color)
            self.frame_canvas.itemconfig(
                self.frame_canvas.vertical_dash_line, fill=cross_hair_color)
            self.adding_box_label.config(
                text="Adding boxes: " + str(not self.frame_canvas.editing))
        elif event.char == "z":
            ...
            self.mode_label.config(
                text="Rolling Labels: " + str(self.frame_canvas.rollingover_label))
            print("r pressed rollover is now",
                  self.frame_canvas.rollingover_label)

        elif event.char == "f":
            SELECTED_CROSS_HAIR_COLOR_INDEX = (
                SELECTED_CROSS_HAIR_COLOR_INDEX + 1) % len(CROSS_HAIR_COLORS)
            print("Changing crosshair to: " +
                  CROSS_HAIR_COLORS[SELECTED_CROSS_HAIR_COLOR_INDEX])
            cross_hair_color = "red" if self.frame_canvas.editing else CROSS_HAIR_COLORS[
                SELECTED_CROSS_HAIR_COLOR_INDEX]
            self.frame_canvas.itemconfig(
                self.frame_canvas.horizontal_dash_line, fill=cross_hair_color)
    ...
    def create_menu(self):
        ...
        fileMenu.add_command(label="Open Directory", underline=1,
                             command=self.open_frames, accelerator="Ctrl+O")

        helpMenu = tk.Menu(menubar, tearoff=False)
        menubar.add_cascade(label="Help", underline=0, menu=helpMenu)
        helpMenu.add_command(label="Help", underline=1,
                             command=self.show_help, accelerator="Ctrl+H")
            ...
            for f in os.scandir(self.file_path):
                if f.is_file():
                    if os.path.splitext(f.name)[1].lower() in extensions:
                        ...
                        # Make sure frames are labeled with 6 digit identifier
                        if len(os.path.splitext(f.name)[0].split("_")[-1]) != 6:
                            correct_file_format = False
            if not correct_file_format:
                print(
                    "Warning: Please make sure images frame file name has 6 digit identifier: <framename>_<xxxxxx>.ext")
            self.frames.sort()
    ...
    def about_app(self):
        ...
        button = tk.Button(about_dialog, text='Close',
                           command=about_dialog.destroy).pack()
